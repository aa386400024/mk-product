...
| 类型         | 命名规则                                     | 示例                                                                         |
| ------------ | -------------------------------------------- | ---------------------------------------------------------------------------- |
| 路由命名     | 使用小写字母和连字符（kebab-case）进行命名。 | `user-profile`, `inventory-management`                                       |
| 路由路径     | 使用小写字母和连字符（kebab-case）。         | `/user-profile`, `/inventory-management`                                     |
| 嵌套路由     | 对于嵌套路由，子路由的路径不需要加 `/`。     | 父路由为 `/user`，子路由可以直接写为 `profile`，完整路径就是 `/user/profile` |
| 命名路由     | 使用驼峰命名法（camelCase）。                | `userInfo`, `inventoryManagement`                                            |
| 组件命名     | 帕斯卡命名法（PascalCase）。                 | `UserProfile`, `ProductDetail`                                               |
| 路由参数     | 使用驼峰命名法（camelCase）。                | `userId`, `productId`                                                        |
| 重定向和别名 | 使用小写字母和连字符（kebab-case）。         | `/user-profile`                                                              |

### 路由懒加载规范

使用懒加载来优化路由性能。以下是懒加载规范：

1. **所有路由使用懒加载**：所有的路由都应使用懒加载。这样，对应的组件只有在路由被访问时才会被加载，可以提高首屏加载速度。

2. **使用 `import()` 语法**：我们使用 JavaScript 的动态 `import()` 语法来实现懒加载。
